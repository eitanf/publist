...
// XML data structure used by Publist, defined as constants rather than
// embedding bits of it all through the code
...
################################################
    ...
    function edit_reference($startkey) {
        ...
        // What we want is (with whitespace \s* stuff removed):
        //    @(<publication>.+?<key>$key</key>.+?</publication>)@
        // but the first .+? is too greedy and will match everything from the
        // first <publication> in the file up to the key.
        //
        // So we use negative look-ahead assertion in the .+? sections to
        // prevent publication tags from being included in the match, thus
        // ensuring that we have only the XML section corresponding to the
        // publication of interest to us right now.
        // We start with the regexp from the J.Friedl's book "Mastering regular
        // expressions" that matches a pair html tags (<tag> and </tag>) without
        // allowing <tag> or </tag> to be within the two tags. (This regexp
        // allows <tag foo=bar> too)
        //    @<tag([^>]*)>(((?!</?tag(?:[^>]*)>).)*)</tag>@si
        //
        // Now, we change this a little bit to allow for additional whitespace
        // within the tags:
        //    @<\s*tag([^>]*)>(((?!<\s*/?\s*tag(?:[^>]*)>).)*)<\s*/\s*tag\s*>@si
        //
        // All that remains is to put the requirement to have <key>$key</key> in
        // there (once again allowing all manner of whitespace in there). The
        // negative lookahead assertion goes on both sides of the $key line to be
        // sure (note: .+? should be fine after $key, but it doesn't seem worth
        // not doing it this way).
        //
        // Note that there have been some reports of PCRE running out of memory
        // if there are too many characters between <tag> and </tag>. The limits
        // seem to be about 2kB between the tags for Win32 systems and 10-12kB
        // between the tags for linux systems so this shouldn't be a problem for PubEdit.
        // See PHP bug 25754 for more details on this:    http://bugs.php.net/bug.php?id=25754

        // Use $this->tag_generate() to actually create the tags programatically
        // as it cuts down on a lot of copy-pasting of code... but it might
        // obfuscate the regexp a little as you don't see it all in once place...
        // but since when were regexps like this one easy  to read anyway?
                    ...
                    // can't keep the data if it doesn't exist, and author arrays should be handled differently
                        if (isset($p->data[$k])) {
                            if (is_array($p->data[$k])) {
                                $this->fields[$k]['qs_vals'] =
                                    array_merge($this->fields[$k]['qs_vals'], $p->data[$k]);
                            } else {
