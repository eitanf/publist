 ...
 *
 * Typical usage:
 *   // create a PubAdmin instance, with the path to the publist installation to be administered
 *   // also works out what action to perform and what action comes next
 *   $pubadmin = new PubAdmin('..', array('pubs.xml', 'books.xml'), 'local.ini', 'admin.ini');
...
class PubAdmin {
	...
	var $DEBUG = 0;        // log level of the class (0..10); higher numbers more verbose
...
################################################
	...
	function PubAdmin ($basepath, $files, $publistini='', $adminini='') {
		// make sure the path ends with the directory separator (/ on a unix box)
		if (substr($basepath, -1) != DIRECTORY_SEPARATOR) {
			$basepath .= DIRECTORY_SEPARATOR;
		}
		$this->basepath   = $basepath;
		$this->publistini = $publistini;
		// make sure that $files is an array (but also permit the user to specify an individual xml file
		// just as "pubs.xml" rather than array("pubs.xml").
		if (! is_array($files)) {
			$files = array($files);
		}
		$this->files      = $files;
		...
		// This class has to do a lot of error handling with the file operations, so 
		// most of these operations will be prefixed with @ to suppress ugly errors. By setting
		// the php.ini option for track_errors, the error messages are available in the magic
		// global $php_errormsg. This can then be used for more elegant and graceful error handling.
		...
		$this->set_actions();
		// the fields to be edited are defined in the field ini file, the PubEdit class does the hard work
		$this->create_editor();
	}

################################################
	///// Start the form for data input, including extra state data hidden within the form
	// the extra info embedded includes:
...
################################################
	...
	// in principle there could be a lot of extra information, some error handling or even
	// a copyright statement emitted from here too
	function form_stop () {
		echo "</form>";
		$this->link_home();
	}
